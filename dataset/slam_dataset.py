...
class SLAMDataset(Dataset):
    def __init__(self, config: Config) -> None:
        ...
        self.cur_speed = 0.0
        self.cur_long_accel = 0.0
        self.cur_lat_accel = 0.0
        self.cur_yaw_rate = 0.0

        self.car_info_files = []
        if self.config.car_info_path and os.path.exists(self.config.car_info_path):
            from natsort import natsorted
            json_files = natsorted([f for f in os.listdir(self.config.car_info_path) if f.endswith('.json')])
            for json_file in json_files:
                timestamp = int(os.path.splitext(json_file)[0])
                self.car_info_files.append((timestamp, os.path.join(self.config.car_info_path, json_file)))
    ...
    def find_closest_json_file(self, pcd_timestamp):
        if not self.car_info_files:
            return None

        # Binary search to find the closest timestamp
        lo, hi = 0, len(self.car_info_files) - 1
        best_idx = -1

        while lo <= hi:
            mid = (lo + hi) // 2
            if self.car_info_files[mid][0] == pcd_timestamp:
                best_idx = mid
                break
            elif self.car_info_files[mid][0] < pcd_timestamp:
                lo = mid + 1
            else:
                hi = mid - 1
        
        if best_idx == -1:
            # After the loop, lo is the insertion point. 
            # The closest element is either at lo-1 or lo.
            if lo == 0:
                best_idx = 0
            elif lo == len(self.car_info_files):
                best_idx = len(self.car_info_files) - 1
            else:
                prev_ts = self.car_info_files[lo-1][0]
                next_ts = self.car_info_files[lo][0]
                if abs(pcd_timestamp - prev_ts) < abs(pcd_timestamp - next_ts):
                    best_idx = lo - 1
                else:
                    best_idx = lo

        return self.car_info_files[best_idx][1]
    ...
    def read_frame_with_loader(self, frame_id, init_pose: bool = True):
        ...
        pcd_filename = os.path.basename(self.loader.scan_files[frame_id_in_folder])
        pcd_timestamp = int(os.path.splitext(pcd_filename)[0])

        closest_json_file = self.find_closest_json_file(pcd_timestamp)

        if closest_json_file and os.path.exists(closest_json_file):
            with open(closest_json_file, 'r') as f:
                data = json.load(f)
                self.cur_speed = data.get("speed", 0.0)
                self.cur_long_accel = data.get("chassisLongAcceleration", 0.0)
                self.cur_lat_accel = data.get("chassisLatAcceleration", 0.0)
                self.cur_yaw_rate = data.get("chassisYawRate", 0.0)
        else:
            self.cur_speed = 0.0
            self.cur_long_accel = 0.0
            self.cur_lat_accel = 0.0
    ...
    def preprocess_frame(self): 
        ...
        if frame_id == 0:  # initialize the first frame, no tracking yet
            ...
            if self.cur_speed > 0.0 or self.cur_yaw_rate != 0.0:
                # use speed to improve initial pose guess
                dt = 0.1 # assume 10hz
                v_x = self.cur_speed
                v_y = 0.0
                ...
                d_x = v_x * dt
                d_y = v_y * dt
                d_yaw = self.cur_yaw_rate * dt

                motion = np.eye(4)
                motion[0, 0] = np.cos(d_yaw)
                motion[0, 1] = -np.sin(d_yaw)
                motion[1, 0] = np.sin(d_yaw)
                motion[1, 1] = np.cos(d_yaw)
                motion[0, 3] = d_x
                motion[1, 3] = d_y

                cur_pose_init_guess = self.last_pose_ref @ motion
